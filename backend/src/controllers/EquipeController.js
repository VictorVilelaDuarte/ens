...
class EquipeController {
  ...
  detalhe(req, res) {
    const { id } = req.params;
    bd.query(
      `SELECT e.*, c.Conselheiro_Perfil, l.Casal_Nome AS Casal_Ligacao, r.Casal_Nome FROM ens_equipe e
        INNER JOIN ens_conselheiro c ON c.Conselheiro_IDMENS = e.Equipe_ConselheiroIDMENS
        INNER JOIN ens_casal AS r ON e.Equipe_CasalRespAtualIDMENS = r.Casal_IDMENS
        INNER JOIN ens_casal AS l ON e.Equipe_CasalLigacaoAtualIDMENS = l.Casal_IDMENS
        WHERE e.Equipe_ID = ${id}`,
      (err, result) => {
        if (err) {
          return res.status(400).json({
            staus: false,
